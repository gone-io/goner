...
type headerNameParser struct {
	gone.Flag
}

func (s *headerNameParser) BuildParser(keyMap map[string]string, field reflect.StructField) (func(context *gin.Context) (reflect.Value, error), error) {
	t := field.Type
	...
	return func(context *gin.Context) (reflect.Value, error) {
		header := context.GetHeader(mainKey)

		if v, err := parser(header); err != nil {
...
func (s *headerNameParser) Name() string {
	return "header"
}
